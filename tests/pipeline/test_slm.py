...
def test_modules_are_imported():
    ...
    for folder in ("slm", "corpus/chat", "corpus/greeter", "tools",
                   "tests/pipeline", "tests/conformance"):
...
def test_composition_is_varied():
    ...
    # not `greetings`: that is the module, and shadowing it here would hide a
    # missing import from the check that looks for exactly that mistake.
    social = {compose.social_exchange(rng)[1] for _ in range(400)}
    check("social replies are varied", len(social) > 60, f"{len(social)} distinct")


def test_greetings_are_varied():
    """The greeter is tiny, so its corpus has to teach a shape rather than a list."""
    rng = random.Random(0)
    drawn = [greetings.greeting(rng) for _ in range(20000)]
    distinct = len(set(drawn))
    check("greetings are mostly distinct", distinct > 12000,
          f"{distinct:,} distinct of 20,000")
    bad = [g for g in drawn if re.search(r"[.!?] [a-z]", g)]
    check("no greeting starts a sentence in lower case", not bad,
          f"{len(bad)} of 20,000, first: {bad[0][:60] if bad else ''}")
    long = [g for g in drawn if len(g) > 170]
    check("greetings stay short enough for an empty page", not long,
          f"{len(long)} over 170 characters")


def test_conversations_are_long_and_varied():
    """Eight or more exchanges on average, with the subject holding across turns."""
    rng = random.Random(0)
    lengths = [len(talk.conversation(rng)) for _ in range(2000)]
    mean = sum(lengths) / len(lengths)
    check("conversations average at least eight exchanges", mean >= 8,
          f"mean {mean:.1f}")
    check("conversation length varies", max(lengths) - min(lengths) > 8,
...
def test_pipeline():
    ...
    try:
        code, out = run("corpus/chat/make_corpus.py", "--target_chars", "60000",
        ...
        code, out = run("slm/train.py", "--data", corpus,
        ...
        code, out = run("slm/standalone.py", "--model", export, "--prompt", "hello",
                        "--max_new", "20")
...
def test_pages_render():
    """Run each page's reply renderer under node - see tests/pages/check_pages.mjs."""
    if subprocess.run(["node", "--version"], capture_output=True).returncode != 0:
        print("  skip  page rendering: node is not installed")
        return
    runner = os.path.join(paths.ROOT, "tests", "pages", "check_pages.mjs")
...
def main():
    ...
    test_greetings_are_varied()
    test_conversations_are_long_and_varied()
