...
HISTORY_PATH = os.path.join(paths.BUILD, "chat_history.json")
...
DIM, PLAIN = "\033[2m", "\033[0m"


def show(response):
    """Print the working dimmed and the reply plainly.

    A model trained without a thinking phase never emits the marker, so it takes the
    first branch and prints exactly as it always did."""
    if THINK_MARK not in response:
        print(f"Bot: {response}\n")
        return
    working, _, said = response.partition(THINK_MARK)
    print(f"{DIM}...: {working.strip()}{PLAIN}")
    print(f"Bot: {said.strip() or '(no reply)'}\n")
